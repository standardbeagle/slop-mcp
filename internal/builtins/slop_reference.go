...
var SlopReference = []SlopFunction{
	...
	// Stored as JSON under the XDG-aware slop-mcp memory directory. Survives restarts. Banks group related keys.
	...
	{Name: "mem_search", Category: "memory", Signature: `mem_search(query, bank: "", include_values: false)`, Description: "Searches across banks by key name and description. Optionally restrict to one bank or include value content in search", Example: `mem_search("review", include_values: true)`, Returns: "list", Notes: "Case-insensitive substring match. Without bank: kwarg, scans every bank in the XDG-aware slop-mcp memory directory. Set include_values: true to also match against serialized JSON of stored values.", Tags: []string{"slop-mcp"}},
